...
@AutoSpellConfig
public class HeavyDesolationSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(DiscerningTheEldritch.MOD_ID, "heavy_desolation");
        ...
        for (int r = 0; r < rings; r++) {
            ...
            for (int i = 0; i < fangs; i++) {
                ...
                if (!world.getBlockState(BlockPos.containing(spawn).below()).isAir()) {
                    // FIX: Ensure entity spawning only happens on server
                    VoidSpikeEntity spike = new VoidSpikeEntity(world, entity, getDamage(spellLevel, entity));
                    spike.moveTo(spawn.x, spawn.y, spawn.z, get2DAngle(center, spawn), 0);
