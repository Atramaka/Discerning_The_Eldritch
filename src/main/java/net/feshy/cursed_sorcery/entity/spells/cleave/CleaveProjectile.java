package net.feshy.cursed_sorcery.entity.spells.cleave;
...
import net.feshy.cursed_sorcery.particle.DTEParticleHelper;
...
public class CleaveProjectile extends PathfinderMob implements AntiMagicSusceptible/*, NoKnockbackProjectile*/ {

    public CleaveProjectile(EntityType<? extends PathfinderMob> pEntityType, Level pLevel) {
    ...
    public CleaveProjectile(Level level, @Nullable Entity owner, @Nullable Entity target, float damage) {
        this(DTEEntityRegistry.CLEAVE.get(), level);
        setOwner(owner);
        setTarget(target);
        this.damage = damage;
    }
    ...
    public void tick() {
        if (this.level().isClientSide) {
            for (int i = 0; i < 2; i++) {
                ...
                this.level().addParticle(DTEParticleHelper.ESOTERIC_SPARKS, getX() + spawn.x, getY() + this.getBbHeight() * .5f + spawn.z, getZ() + spawn.z, motion.x, motion.y, motion.z);
            }
        }
        super.tick();
    ...
    protected void customServerAiStep() {
        ...
        if (this.tickCount % 8 == 0) {
            if (this.level().collidesWithSuffocatingBlock(this, this.getBoundingBox().move(0, -1, 0))) {
            ...
            this.level().getEntities(this, this.getBoundingBox().inflate(inflate), this::canHitEntity).forEach(
                    (entity) -> {
                        if (canHitEntity(entity)) {
                            boolean hit = DamageSources.applyDamage(entity, damage, SpellRegistries.CLEAVE.get().getDamageSource(this, getOwner()));
                            if (hit) {
