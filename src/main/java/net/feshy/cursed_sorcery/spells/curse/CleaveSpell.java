...
public class CleaveSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(CursedSorcery.MOD_ID, "cleave");
    ...
    public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        float distance = 8f; //todo: scale with power
        ...
        if (!targetableEntities.isEmpty() && targetableEntities.get(0).distanceToSqr(entity) < distance * distance) {
            ...
            for (Entity targetEntity : damageEntities) {
                    ...
                    if (DamageSources.applyDamage(targetEntity, getDamage(spellLevel, entity), damageSource)) {
                        MagicManager.spawnParticles(level, ParticleTypes.LARGE_SMOKE, targetEntity.getX(), targetEntity.getY() + targetEntity.getBbHeight() * .5f, targetEntity.getZ(), 15, targetEntity.getBbWidth() * .5f, targetEntity.getBbHeight() * .5f, targetEntity.getBbWidth() * .5f, .25, false);
...
//                        targetEntity.setDeltaMovement(targetEntity.getDeltaMovement().add(end.subtract(targetEntity.position()).scale(1 / 6f)));
                        Vec3 knockback = targetEntity.position().subtract(entity.position()).normalize().add(0, 0.2, 0).normalize();
                        ...
                        // Apply the cleave effect to the target
...
//                particlePos.x, particlePos.y + .3, particlePos.z, 1, 0, 0, 0, 0, true);
        int trailParticles = 20;
        double speed = rayVector.length() / 12.0 * -0.3;
        for (int i = 0; i < trailParticles; i++) {
            ...
            MagicManager.spawnParticles(level, new TraceParticleOptions(Utils.v3f(particleEnd), new Vector3f(.6f, 0f, 0f)), particleStart.x, particleStart.y, particleStart.z, 1, 0, 0, 0, speed, false);
        }
